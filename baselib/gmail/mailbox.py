...
class Mailbox:
    ...
    def __init__(self, gmail, name="INBOX"):
        ...
        # self.external_name = name.encode()
        self.gmail = gmail
    ...
    def mail(self, prefetch=False, **kwargs):
        ...
        kwargs.get('after') and search.extend(
            ['SINCE',
             kwargs.get('after').strftime(self.date_format)
             ]
        )
        kwargs.get('on') and search.extend(
            ['ON',
             kwargs.get('on').strftime(self.date_format)
             ]
        )

        kwargs.get('header') and search.extend(
            ['HEADER',
             kwargs.get('header')[0], kwargs.get('header')[1]
             ]
        )
        ...
        if response == 'OK':
            ...
            uids = filter(None, data[0].decode('utf-8').split(' '))
            # newdata = data.decode('utf-8')
            # filter out empty strings
            # uids = filter(None, newdata[0].split(' '))
